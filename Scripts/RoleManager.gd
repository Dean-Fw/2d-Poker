...
func _set_small_blind() -> void:
	if small_blind == null:
		...
		small_blind = _set_role(small_blind)
...
func _set_role(role: Player) -> Player:
	if _active_players.find(role) + 1 > _active_players.size() - 1:
		return _active_players[0]
	else:
		return _active_players[_active_players.find(role) + 1]
